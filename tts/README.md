...
- In windowed mode, history keeps an update at its original generation time even when replayed. Fresh updates use compact relative times for their first day, then an absolute timestamp. Search narrows the visible history, each update can be archived with one click, and the Archive control switches between active and archived updates.
