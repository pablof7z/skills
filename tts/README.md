...
- The transcript shows only the agent's message while the subject remains audible. It preserves paragraphs and lists, renders common Markdown structure, and keeps a natural phrase softly in focus using real synthesis timestamps, including while the user scrolls. Its prose geometry stays fixed, phrase following scrolls only when needed, and every mapped word remains clickable for seeking without a separate word-level playback highlight.
...
- History keeps an update at its original generation time even when replayed. Fresh updates use compact relative times for their first day, then an absolute timestamp. The native titlebar toolbar keeps search beside an additive project-and-agent filter whose badge shows the active selection count. Projects and session-aware agents form one inclusive filter, and either identity can be clicked directly on a row. The menu also switches between recent and archived updates. Swipe left on an update to archive it, or restore it from the archived view.
...
available as an explicit per-process override. Each generation attempt has a
shared 120-second deadline, including time waiting for a slot; set
`TTS_GENERATION_TIMEOUT_SECONDS` to a positive integer to override it.
