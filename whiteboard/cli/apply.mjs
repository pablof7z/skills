...
// { rev, id, title, ops } on success; throws on any failure (caller surfaces it).
export function applyOps(session, { title, ops, summary = null, by, piSessionId = null }) {
  ...
  const ch = appendChange(session.dir, { id: changeIdFor(title), title, by: by || agentName(), summary, ops: built, via });
  return { rev: ch.rev, id: ch.id, title: ch.title, ops: ch.ops.length };
}
